...
Для лабораторных Jenkins job используется SSH node `labs`:
```
Host labs
  HostName 192.168.24.19
  User ubuntu
  IdentityFile /var/jenkins_home/.ssh/id_ed25519_labs
```

На этом сервере производится сборка и архивация артефакта. Параметры node можно изменить при запуске корневого `Jenkinsfile`.
